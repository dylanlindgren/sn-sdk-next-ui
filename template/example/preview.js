...
function injectStyles() {
  if (document.getElementById("sn-sdk-next-ui-preview-styles")) return;
  const style = document.createElement("style");
  style.id = "sn-sdk-next-ui-preview-styles";
  style.textContent = `
...
export function renderPreview({ manifest = {}, previews = {} } = {}) {
  const run = () => {
    ...
    if (tags.length === 0) {
      ...
      empty.textContent =
        "No components yet. Run `sn-sdk-next-ui add` to create one, then reload.";
      root.append(empty);
    } else {
