...
// declaring a neutral `sdkNowUiPlugin` field in its package.json pointing at the
...
async function loadPluginFromDir(dir) {
  ...
  const entryRel = pkg.sdkNowUiPlugin;
...
async function loadPluginSpec(spec, cwd) {
  ...
  if (!descriptor) {
    throw new Error(
      `Plugin "${spec}" is not a valid plugin (no "sdkNowUiPlugin" field in its package.json).`,
    );
  }
  return descriptor;
}
...
export async function init({ argv = [] } = {}) {
  ...
  try {
    ...
    if (await exists(path.join(cwd, "now-ui.json"))) {
      p.cancel(
        "This project already looks initialised (now-ui.json exists). Use `sdk-now-ui add` to create components.",
      );
      process.exit(1);
    }
    ...
    p.outro("Done! Add a component with: " + pm + " sdk-now-ui add");
  } catch (error) {
...
export async function add({ argv = [] } = {}) {
  const cwd = process.cwd();
  p.intro("sdk-now-ui add");

  try {
    const nowUiPath = path.join(cwd, "now-ui.json");
    if (!(await exists(nowUiPath))) {
      p.cancel("No now-ui.json found. Run `sdk-now-ui init` first.");
      process.exit(1);
    }
